...
labs(title = latex2exp::TeX("Null Distribution with $\\mu$ = 5 (null hypothesis)"), y = "Density") +
annotate("text", x = 8.7, y = 0.15, size = 10, col = "red", label = latex2exp::TeX("$\\alpha$")) + xlim(-3, 12)  
Null.Plot
```
...
mutate(y = dnorm(x, mean = 11, sd = xbar))
Alt.Plot <- ggplot(Error.df, aes(x = x, y = y)) + geom_area(fill = "grey90") +
geom_area(data = Error.shade, aes(x = x, y = y), fill = "grey25") +
geom_line(size = 2) +
geom_vline(xintercept = xbar.threshold, col = "black", linetype = "dashed", size = 1.5) +
labs(title = latex2exp::TeX("Alternative Distribution with $\\mu$ = 11 (alternative hypothesis)"), y = "Density") +
annotate("text", x = 7.8, y = 0.10, size = 10, col = "red", label = latex2exp::TeX("$\\beta$")) + xlim(0, 18)  
Alt.Plot
```
Value of $\beta$ when using the z-distribution:
```{r}
pnorm(8.35, mean = 11, sd = 5/sqrt(8))
```

Value of $\beta$ when using the t-distribution:
```{r}
pt(8.35, df = 7)
```
