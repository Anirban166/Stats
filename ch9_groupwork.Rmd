...
The Shpiro-Wilk's test shows that the residuals are not normal because the p-value is much smaller than standard significance 0.05.
...
ggplot(iris_df, aes(x=Species, y=Sepal.Width)) + geom_boxplot()
autoplot(model, which=c(1,2))
```
...
```{r}
car::leveneTest(Sepal.Width ~ Species, data=iris_df)
```
