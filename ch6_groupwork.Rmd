...
pvalue
```
We obtained a p-value > 0.05, which is not statistically significant and indicates strong evidence for the null hypothesis, i.e. this suggests the mean oxygen level can indeed be less than 5ppm, or at least the fact that we cannot reject our null hypothesis from the taken test statistic.
...
```{r}
# under same alternative value of mu = 11, find the probability of a Type II error. AKA calculate: Beta = P(Xbar < 8.35 | mu = 11)
...
```{r}
power.t.test(delta=5, sd=20, sig.level=0.1, power=0.9, 
             type="one.sample", alternative="one.sided")
```

Sample size should be about 106.

## 4b

```{r}
power.t.test(delta=10, sd=20, sig.level=0.1, power=0.9, 
             type="one.sample", alternative="one.sided")
```

Sample size should be around 28. Comment on diff
